...
/* global c, d, dbe, k, l, Response, times */
/* exported arraymax, arraymin, byId, cfetch, fetchasync, immutable, handleKey, isBlank, isEmptyData, isFunction, isIterable, isloaded, isNil, isNumber, isObject, isPrimitive, isVisible, loadstyles, memoize, objectsize, plural, removeAllEventListener, resourceisloaded, selectItem, sleep, sizeOf, sortobjectbykey, sortobjectbyvalue */
...
String.prototype.na = function() { return this.normalize('NFD').replace(/[\u0300-\u036f]/g, ""); };
...
String.prototype.abbrev = function(n = 3) {
	return this.substr(0, n);
}

String.prototype.getposttype = function() {
	let pty = [];
...
Array.prototype.groupBy = function(i, excludeundefined = false) {
...
Array.prototype.groupByMultiple = function(values, excludeundefined = false) {
	let list = this.slice();
...
Array.prototype.countByMultiple = function(values, excludeundefined = false) {
	let list = this.slice();
	if (!values.length) return list;
	let byFirst = list.groupBy(values[0], excludeundefined);
	let rest = values.slice(1);
...
Array.prototype.sortBy = function(attrs) {
...
Array.prototype.sortlocale = function(field) {
	let array = this.slice();
...
Array.prototype.uniqueby = function (predicate) {
...
Array.prototype.gaussiansort = function(index = null) {
	let _a = this.slice();
...
Array.prototype.quantile = function(quant) {
...
Array.prototype._intersection = function(...a) {
	return [this, ...a].reduce((p, c) => p.filter(e => c.includes(e)) || c);
};

Array.prototype._difference = function(...a) {
	return [this, ...a].reduce((p, c) => p.filter(e => !c.includes(e)));
};

Array.prototype._union = function(...a) {
...
if (!String.prototype.startsWith) {
	String.prototype.startsWith = function(searched, position) {
		position = position || 0;
...
function *cartesianproduct(...arrays) {
	if (!arrays.length) {
		...
		var [tail, ...head] = arrays.reverse();
		var beginning = cartesianproduct(...head.reverse());
		for(var b of beginning) for(var t of tail) yield b + t;
	}
}
